...
def mock_validate_valid_ticket(mocker, user):
    return mocker.patch(
        "django_ent.middleware.CASMiddleware.validate_ticket",
        return_value=user.entinstitution.uai,
    )
...
def mock_validate_invalid_ticket(mocker):
    return mocker.patch(
        "django_ent.middleware.CASMiddleware.validate_ticket", return_value=[]
    )
